...
import { Confession, ConfessionStatus, Database, ConfessionWithProfile, Json } from '@/types/database'

type ConfessionUpdate = Database['public']['Tables']['confessions']['Update']
...
export default function ModerationTable({ confessions, onUpdate }: ModerationTableProps) {
    ...
    const handleStatusChange = async (id: string, status: ConfessionStatus) => {
        ...
        try {
            const updateData: ConfessionUpdate = { status }
            if (status === 'approved') {
    ...
    const getContentPreview = (content: Json): string => {
        try {
            const parsed = typeof content === 'string' ? JSON.parse(content) : content
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            return (parsed as any).content
                ?.map((node: Record<string, unknown>) => 
                    (node.content as Array<Record<string, unknown>>)?.map((c) => c.text || '').join('') || ''
                )
                .join(' ')
            ...
            <AnimatePresence>
                {viewConfession && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        >
                            <ConfessionCard
                                confession={viewConfession as ConfessionWithProfile}
                                index={0}
